...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Plot presentation-ready figures from AstroDSB magnetic evaluation artifacts."
    )
    parser.add_argument(
        "--results-dir",
        type=Path,
        default=Path("results/mag_run"),
        help="Directory containing mag_run outputs.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def log(message):
    print(f"[plot_magnetic_results] {message}")
...
def plot_training_progress_grid(results_dir: Path, output_dir: Path, dpi: int, sample_index: int):
    ...
    fig.suptitle("Magnetic Training Progress", fontsize=14)
...
def plot_final_eval_examples(
    ...
    fig.suptitle("Final Magnetic Evaluation Examples", fontsize=14)
