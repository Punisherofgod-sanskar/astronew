...
def setup_loader(dataset, batch_size, num_workers=4, *, shuffle=True, drop_last=True):
    loader = DataLoaderX(
        dataset,
...
def build_dataloader(dataset, batch_size, num_workers=4, *, shuffle=False, drop_last=False):
    return DataLoaderX(
        dataset,
        batch_size=batch_size,
        shuffle=shuffle,
...
def build_dataloader_with_sampler(dataset, batch_size, sampler, num_workers=4, *, drop_last=False):
    return DataLoaderX(
        dataset,
        batch_size=batch_size,
        sampler=sampler,
    ...
    def __init__(self, opt):
        ...
        if self.rank == 0:
            if not wandb_is_available():
                raise ImportError("wandb is not installed but --log-writer wandb was requested")
            login_kwargs = {}
            if getattr(opt, "wandb_api_key", None):
                login_kwargs["key"] = opt.wandb_api_key
            wandb.login(**login_kwargs)
            wandb.init(
                dir=str(opt.log_dir),
                project=getattr(opt, "wandb_project", "astrodsb"),
